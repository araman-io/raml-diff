...
/**
 * The class finds out the Response that are modified.
 * 
 *
 */
public class FindActionsWithUpdatedResponseSchema implements Finder {
   ...
   * Response Objects whose Schema has been modified as UPDATED.
   * 
        ...
        commonResponses
            ...
            .flatMap(
                responseKey -> {
                  ...
                  List<String> allResponseSchemaDifferences = new ArrayList<String>();

                  addMimeTypeDifferenceDetails(allResponseSchemaDifferences, responseKey, newlyAddedMimeTypes, IS_ADDED_TEXT);
                  addMimeTypeDifferenceDetails(allResponseSchemaDifferences, responseKey, deletedMimeTypes, IS_DELETED_TEXT);
                  ...
                  for (String mimeType : existingMimeTypes) {
                    ...
                    if (!StringUtils.equals(newMimeType.getSchema(), oldMimeType.getSchema())) {
                      addChangedMimeType(allResponseSchemaDifferences, responseKey, IS_UPDATED_TEXT, mimeType);
                    }
                  }

                  return allResponseSchemaDifferences.stream();
                }).collect(Collectors.toList());
